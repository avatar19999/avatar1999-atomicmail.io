        ...
        <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-5">
            ...
            <div class="space-y-4 text-xs">
                <label class="flex items-center gap-3 cursor-pointer">
                    ...
                    <span class="font-bold text-slate-200">فعال‌سازی بازنویسی هوشمند متن قبل از ارسال</span>
                </label>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block font-medium text-slate-300 mb-1">سرویس‌دهنده هوش مصنوعی:</label>
                        <select name="ai_provider" class="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5 text-white focus:border-cyan-500 focus:outline-none">
                            <option value="gemini" <?= ($aiConfig['provider'] ?? '') === 'gemini' ? 'selected' : '' ?>>Google Gemini (پیشنهادی)</option>
                            <option value="openrouter" <?= ($aiConfig['provider'] ?? '') === 'openrouter' ? 'selected' : '' ?>>OpenRouter</option>
                        </select>
                    </div>
                    <div>
                        <label class="block font-medium text-slate-300 mb-1">مدل (Model):</label>
                        <input type="text" name="ai_model" value="<?= htmlspecialchars($aiConfig['model'] ?? 'gemini-1.5-flash') ?>" class="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5 text-white font-mono focus:border-cyan-500 focus:outline-none">
                    </div>
                    <div class="sm:col-span-2">
                        <label class="block font-medium text-slate-300 mb-1">کلید API هوش مصنوعی (API Key):</label>
                        <input type="password" name="ai_api_key" value="<?= htmlspecialchars($aiConfig['api_key'] ?? '') ?>" class="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5 text-white font-mono focus:border-cyan-500 focus:outline-none" placeholder="AIzaSy... یا sk-or-...">
                    </div>
                </div>
            </div>
        </div>
