...
async function runAiRewrite(
  ...
  if (!apiKey || !apiKey.trim()) {
    console.log('[AI] OpenRouter API Key Missing');
    return text;
  }

  const styleVal = styleOverride || config.rewrite_style || config.rewriteStyle || config.writingStyle || 'formal';
  const isFriendly = styleVal === 'friendly';
  const styleLog = isFriendly ? 'Friendly' : 'Formal';

  console.log('[AI] Rewrite Enabled');
  console.log(`[AI] Style: ${styleLog}`);

  const model = (process.env.OPENROUTER_MODEL && process.env.OPENROUTER_MODEL.trim())
    ? process.env.OPENROUTER_MODEL.trim()
    : 'deepseek/deepseek-chat';

  const systemPrompt = `تو یک ویراستار حرفهای فارسی هستی.

قوانین:
- متن را حدود ۳۰ تا ۴۰ درصد بازنویسی کن.
- مفهوم اصلی را تغییر نده.
- لحن را بر اساس Rewrite Style تنظیم کن.
- اگر حالت "رسمی و خبری" انتخاب شده بود، متن کاملاً رسمی و مناسب خبر باشد.
- اگر حالت "دوستانه" انتخاب شده بود، متن روان و صمیمی باشد.
- ایموجی اضافه یا حذف نکن.
- لینکها، شمارهها، یوزرنیمها و هشتگها را حفظ کن.
- فقط متن نهایی را برگردان و هیچ توضیحی ننویس.`;

  const userContent = `[Rewrite Style: ${isFriendly ? 'دوستانه' : 'رسمی و خبری'}]\n\n${text}`;
...
async function runAiJobExtraction(text: string): Promise<{
  ...
  if (!process.env.GEMINI_API_KEY) {
    ...
    const company = (text.match(/(?:شرکت|مجموعه|گروه)\s+([^\n،,]+)/i)?.[1] || 'مجموعه معتبر').trim();
    const location = (text.match(/(?:تهران|کرج|اصفهان|شیراز|مشهد|موقعیت|شهر)\b[^\n،,]*/i)?.[0] || 'تهران').trim();
    const salary = (text.match(/(?:حقوق|مزایا|درآمد)\s*[:\s]*([^\n،,]+)/i)?.[1] || 'توافقی').trim();
    const contact = (text.match(/(?:@\w+|09\d{9}|\+98\d{10})/i)?.[0] || 'ارتباط با آیدی یا شماره مندرج').trim();
    const skills = 'مسلط به مهارت‌های تخصصی مرتبط';
    const deadline = 'تا تکمیل ظرفیت';
  ...
  try {
    const prompt = `تو یک پردازشگر هوشمند آگهی‌های شغلی به زبان فارسی هستی. متن آگهی زیر را تحلیل کن و اطلاعات آن را به صورت JSON معتبر با کلیدهای زیر برگردان:
{
  "jobTitle": "عنوان شغلی",
  "company": "نام شرکت یا مجموعه",
  "location": "شهر یا موقعیت مکانی",
  "skills": "مهارت‌های اصلی",
  "salary": "حقوق و مزایا",
  "contact": "اطلاعات تماس، آیدی یا شماره تلفن",
  "deadline": "مهلت ارسال رزومه یا ثبت‌نام"
}
...
${text}`;
    ...
    rawStr = rawStr.replace(/```json/g, '').replace(/```/g, '').trim();
    let parsed: any = {};
    try {
    ...
    const jobTitle = parsed.jobTitle || 'استخدام نیروی متخصص';
    const company = parsed.company || 'مجموعه معتبر';
    const location = parsed.location || 'نامشخص';
    const skills = parsed.skills || 'سوابق مرتبط';
    const salary = parsed.salary || 'توافقی';
    const contact = parsed.contact || 'ارتباط با مدیر';
    const deadline = parsed.deadline || 'تا تکمیل ظرفیت';
    ...
    console.error('AI Job Extraction error:', err);
    return {
      extracted: {
        jobTitle: 'فرصت شغلی',
...
async function runMonitoringHealthCheck() {
  ...
  for (const source of activeSources) {
    ...
    try {
      if (!source.id) {
        console.log(`[HEALTH] ${channelName} ERROR: Database record missing ID`);
        errCount++;
        continue;
      }
      ...
      if (isCoverageActive || (gramClient && gramStatus === "connected")) {
        ...
        console.log(`[HEALTH] ${channelName} WARNING: Telegram client offline`);
        okCount++;
      }
    } catch (err: any) {
      console.log(`[HEALTH] ${channelName} ERROR: ${err?.message || err}`);
      errCount++;
    }
  }

  console.log(`[HEALTH SUMMARY] Total Active: ${activeSources.length} | OK: ${okCount} | ERRORS: ${errCount}`);
  console.log("==========================================");
}
...
async function sendTelegramConnectionSuccessNotification(botUsername: string, destChannel: string, dbInfo: string) {
  ...
  const timeFa = new Date().toLocaleTimeString("fa-IR");
...
// Telegram Database Backup Service (Sends backup_YYYY-MM-DD_HH-mm-ss.dump document directly to Telegram)
async function performTelegramDatabaseBackup(
  ...
  const dest = customDestination ? String(customDestination) : store.settings.destinationChannel;

  if (!token) {
    return { success: false, message: "توکن ربات تلگرام هنوز تنظیم نشده است." };
  }
  if (!dest) {
    return { success: false, message: "کانال یا گروه مقصد برای ارسال بک‌آپ تنظیم نشده است." };
  }
  ...
  try {
    ...
    const caption = isManualTest
      ? `🧪 <b>تست پشتیبان‌گیری فوری از دیتابیس ربات</b>\n\n` +
        `📦 فایل: <code>${filename}</code>\n` +
        `📅 تاریخ و زمان: <code>${dateFa} ${timeFa}</code>\n` +
        `🗄️ وضعیت دیتابیس: <b>${isDbConnected ? "PostgreSQL متصل" : "پایگاه داده محلی"}</b>\n` +
        `📊 کانال‌های ثبت‌شده: <b>${store.sources.length}</b>\n` +
        `📦 حجم فایل: <b>${(buffer.length / 1024).toFixed(1)} KB</b>\n\n` +
        `✅ این فایل رسمی <code>backup.dump</code> را می‌توانید جهت بازنشانی در پنل مدیریت یا ارسال به همین ربات استفاده کنید.`
      : `📦 <b>پشتیبان‌گیری خودکار ۲۴ ساعته از دیتابیس ربات</b>\n\n` +
        ...
        `🔒 این فایل رسمی <code>backup.dump</code> شامل نسخه کامل تمامی اطلاعات، تنظیمات و کانال‌های سیستم می‌باشد و از طریق ربات و پنل قابل بازنشانی (Restore) است.`;
...
async function startServer() {
  ...
  // Requirement: Health check routes for Railway / Uptime monitoring
  ...
  app.post("/api/ai-processing/test-rewrite", async (req, res) => {
    ...
    try {
      const rewrittenText = await runAiRewrite(text, style, true);
  ...
  function getBotAdminMainMenuContent() {
    ...
    const text =
      ...
      `🕒 <b>زمان سرور:</b> <code>${new Date().toLocaleTimeString("fa-IR")}</code>\n\n` +
  ...
  function getBotAdminStatusContent() {
    ...
    const text =
      ...
      `• <b>حالت ضد تحریم کلودفلر:</b> ${store.cloudflareConfig?.enabled ? "🟢 فعال" : "⚪ غیرفعال"}\n\n` +
      `🕒 آخرین بررسی: <code>${new Date().toLocaleTimeString("fa-IR")}</code>`;
  ...
  function getBotAdminChannelsContent() {
    ...
    // Create delete buttons for each channel (max 10 to fit telegram limits)
  ...
  function getBotAdminFiltersContent() {
    ...
    const text =
      `🧹 <b>تنظیمات پردازش، پاکسازی و فیلتر پیام‌ها</b>\n\n` +
      `• <b>پاکسازی لینک‌ها و منشن‌ها:</b> ${cleanEnabled ? "🟢 فعال" : "🔴 غیرفعال"}\n` +
      `• <b>فیلتر کلمات مجاز/ممنوع:</b> ${kwEnabled ? "🟢 فعال" : "🔴 غیرفعال"}\n` +
      `• <b>امضای اختصاصی انتهای پست:</b> ${sigEnabled ? "🟢 فعال" : "🔴 غیرفعال"}\n\n` +
      `<i>با کلیک روی هر دکمه، وضعیت آن فوراً تغییر می‌کند:</i>`;

    const reply_markup = {
      inline_keyboard: [
        [
          {
            text: cleanEnabled ? "🧹 پاکسازی محتوا: [فعال ✅]" : "🧹 پاکسازی محتوا: [غیرفعال ❌]",
            callback_data: "cb_toggle_clean",
          },
        ],
        [
          {
            text: kwEnabled ? "🔎 فیلتر کلمات: [فعال ✅]" : "🔎 فیلتر کلمات: [غیرفعال ❌]",
            callback_data: "cb_toggle_kw",
          },
        ],
        [
          {
            text: sigEnabled ? "✍️ امضای پیام: [فعال ✅]" : "✍️ امضای پیام: [غیرفعال ❌]",
  ...
  function getBotAdminLogsContent() {
    ...
    let text = `📑 <b>آخرین گزارشات فعالیت و انتقال پیام</b>\n\n`;
    ...
    if (logs.length === 0) {
      ...
      logs.forEach((l, idx) => {
        ...
        const time = new Date(l.timestamp).toLocaleTimeString("fa-IR");
        ...
        for (const update of updatesRes.result) {
          ...
          if (update.callback_query) {
            ...
            if (data === "cb_main_menu") {
              ...
              await callTelegramBotApi(token, "answerCallbackQuery", {
                callback_query_id: cq.id,
                text: "⏳ در حال استخراج دیتابیس و تولید backup.dump...",
              });
              const backupRes = await performTelegramDatabaseBackup(true, chatId);
              if (backupRes.success) {
                await callTelegramBotApi(token, "sendMessage", {
                  chat_id: chatId,
                  text: `✅ فایل نسخه پشتیبان (<code>${backupRes.filename}</code>) با موفقیت تولید و برای شما ارسال شد.`,
                  parse_mode: "HTML",
                });
              } else {
                await callTelegramBotApi(token, "sendMessage", {
                  chat_id: chatId,
                  text: `❌ خطا در تهیه بک‌آپ: ${backupRes.message}`,
                });
              }
          ...
          if (update.message && update.message.text) {
            ...
            if (text === "/start" || text === "/menu" || text === "/admin") {
              ...
              if (dest) {
                await callTelegramBotApi(token, "sendMessage", {
                  chat_id: dest,
                  text: `🧪 پیام تست از تلگرام به مقصد (${new Date().toLocaleTimeString("fa-IR")})`,
                });
                await callTelegramBotApi(token, "sendMessage", {
                  chat_id: chatId,
              ...
              await callTelegramBotApi(token, "sendMessage", {
                chat_id: chatId,
                text: "⏳ در حال استخراج کامل اطلاعات و ایجاد فایل backup.dump...",
              });
              const backupRes = await performTelegramDatabaseBackup(true, chatId);
              if (backupRes.success) {
                await callTelegramBotApi(token, "sendMessage", {
                  chat_id: chatId,
                  text: `✅ فایل نسخه پشتیبان (<code>${backupRes.filename}</code>) با موفقیت ارسال گردید.`,
                  parse_mode: "HTML",
                });
              } else {
                await callTelegramBotApi(token, "sendMessage", {
              ...
              await callTelegramBotApi(token, "sendMessage", {
                chat_id: chatId,
                text:
                  `📖 <b>راهنمای دستورات مدیریتی ربات تلگرام</b>\n\n` +
                  `• /menu یا /start - نمایش منوی تعاملی شیشه‌ای\n` +
                  `• /status - دریافت آمار و وضعیت مانیتورینگ\n` +
                  `• /backup - دریافت فوری فایل نسخه پشتیبان (backup.dump)\n` +
                  `• /restore - بارگذاری و بازیابی فایل بک‌آپ\n` +
                  `• /pause - توقف موقت مانیتورینگ\n` +
                  `• /resume - شروع و اتصال مجدد مانیتورینگ\n` +
            ...
            try {
              ...
              if (fileName.endsWith(".sql")) {
                if (pool && isDbConnected) {
                  ...
                  throw new Error("پایگاه داده PostgreSQL متصل نیست. لطفاً فایل backup.dump را ارسال فرمایید.");
                }
              } else {
      ...
      if (action === "menu") {
        ...
        if (dest && token) {
          const r = await callTelegramBotApi(token, "sendMessage", {
            chat_id: dest,
            text: `🧪 پیام تست ارسالی از شبیه‌ساز مدیریت (${new Date().toLocaleTimeString("fa-IR")})`,
          });
