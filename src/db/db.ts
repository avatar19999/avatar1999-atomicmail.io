...
async function initializeDefaultRecordsInPostgres() {
  ...
  // 1. settings
  await pool.query(`
    INSERT INTO settings (id, bot_token, destination_channel, admin_password_hash, is_monitoring_paused, is_verified, cloudflare_config)
    VALUES ('default', '', '', 'admin123', false, false, '{"enabled": true, "mode": "auto", "workerUrl": ""}')
  ...
  await pool.query(`
    ...
    VALUES (
      'default', true, true, 'gemini-2.5-flash', '', false, true, false,
  ...
  try {
    ...
    if (pool) {
      ...
      try {
        ...
        await client.query(
          `INSERT INTO settings (id, bot_token, destination_channel, admin_password_hash, is_monitoring_paused, cloudflare_config)
           ...
           cloudflare_config = EXCLUDED.cloudflare_config`,
          [
            encryptedBotToken,
            store.settings?.destinationChannel || '',
            store.adminPasswordHash || 'admin123',
            !!store.isMonitoringPaused,
            JSON.stringify(store.cloudflareConfig || { enabled: true, mode: 'auto', workerUrl: '' }),
          ]
        );
...
export async function getStoreFromDb(): Promise<any> {
  ...
  const aiProcessingConfig = {
    ...
    rewrite_style: aiRow.rewrite_style || aiRow.writing_style || 'formal',
    rewriteStyle: aiRow.rewrite_style || aiRow.writing_style || 'formal',
  ...
  return {
    ...
    cloudflareConfig: settingsRow.cloudflare_config || { enabled: true, mode: 'auto', workerUrl: '' },
    settings: {
      botToken,
  ...
  try {
    ...
    await client.query(
      `INSERT INTO settings (
        ...
        rate_limits, panel_config, forwarding_settings, cloudflare_config, updated_at
      )
      ...
      [
        ...
        JSON.stringify(settingsData.cloudflareConfig || { enabled: true, mode: 'auto', workerUrl: '' }),
      ]
    );
...
export async function exportDatabaseSql(includeSecrets = false, fallbackStore?: any) {
  ...
  // Settings
  sql += `-- Settings\n`;
  sql += `INSERT INTO settings (id, bot_token, destination_channel, admin_password_hash, is_monitoring_paused, cloudflare_config)
VALUES ('default', '${data.settings?.botToken || ''}', '${data.settings?.destinationChannel || ''}', '${data.adminPasswordHash || ''}', ${!!data.isMonitoringPaused}, '${JSON.stringify(data.cloudflareConfig || {})}')
ON CONFLICT (id) DO UPDATE SET bot_token = EXCLUDED.bot_token, destination_channel = EXCLUDED.destination_channel;\n\n`;

  // Sources
  sql += `-- Sources (${data.sources?.length || 0} items)\n`;
  if (Array.isArray(data.sources)) {
...
VALUES ('${src.id}', '${(src.title || '').replace(/'/g, "''")}', '${(src.name || '').replace(/'/g, "''")}', '${src.username || ''}', '${src.type || 'channel'}', '${src.status || 'active'}', ${src.isChannel !== false}, ${src.isActive !== false}, '${JSON.stringify(src.keywordFilter || null)}', '${JSON.stringify(src.contentCleaner || null)}', '${(src.signature || '').replace(/'/g, "''")}', '${JSON.stringify(src.cleaningRules || null)}', '${(src.footerText || '').replace(/'/g, "''")}')
