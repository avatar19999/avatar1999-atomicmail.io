...
export async function testAiRewrite(text: string, style: string, customStyle?: string): Promise<{ success: boolean; rewrittenText: string; message?: string }> {
  return safeFetchJson("/api/ai-processing/test-rewrite", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ text, style, customStyle }),
  }, { success: false, rewrittenText: text });
}
...
// Cloudflare Worker Generator & Toggle APIs
export async function getCloudflareWorkerCode(): Promise<{ success: boolean; code: string; configSummary?: any }> {
  return safeFetchJson("/api/cloudflare/generate-code", undefined, { success: false, code: "" });
}

export async function setCloudflareWebhook(workerUrl: string): Promise<{ success: boolean; message: string; result?: any }> {
  return safeFetchJson("/api/cloudflare/set-webhook", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function getCloudflareStatus(): Promise<{
  success: boolean;
  cloudflareConfig: { enabled: boolean; mode: 'always' | 'auto' | 'disabled'; workerUrl: string };
  webhookActive: boolean;
}> {
  return safeFetchJson("/api/cloudflare/status", undefined, {
    success: false,
    cloudflareConfig: { enabled: true, mode: 'auto', workerUrl: '' },
    webhookActive: false,
  });
}

export async function toggleCloudflareConfig(payload: {
  enabled?: boolean;
  ...
  return safeFetchJson("/api/cloudflare/toggle", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function deleteCloudflareWebhook(): Promise<{ success: boolean; message: string }> {
  return safeFetchJson("/api/cloudflare/delete-webhook", {
    method: "POST",
  }, { success: false, message: "" });
}
